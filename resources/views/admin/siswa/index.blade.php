                    ...
                    <form id="filterForm" method="GET" action="{{ route('admin.siswa.index') }}" class="mb-4">
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-4 relative" x-data="{ open: false }" @click.away="open = false">
                                <button type="button" @click="open = !open" class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-indigo-500">
                                    Status
                                    @if(request('status'))
                                        <span class="ml-1.5 px-1.5 py-0.5 bg-indigo-100 text-indigo-700 rounded-full text-xs font-semibold">
                                            ({{ ucfirst(request('status')) }})
                                        </span>
                                    @endif
                                    <svg class="w-4 h-4 ml-1.5 transition-transform duration-200" :class="{'transform rotate-180': open}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div x-show="open"
                                     ...
                                     class="absolute z-20 mt-1 w-56 origin-top-right right-0 bg-white rounded-md shadow-xl ring-1 ring-black ring-opacity-5 p-3"
                                     style="display: none;">
                                    <h4 class="font-semibold text-gray-800 mb-2 text-sm">Filter Status</h4>
                                    <div class="space-y-1">
                                        <label class="flex items-center text-gray-700 text-sm hover:bg-gray-100 px-2 py-1.5 rounded cursor-pointer">
                                            <input type="radio" name="status" value="" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                                                   {{ !request('status') ? 'checked' : '' }}>
                                            <span class="ml-2">Semua Status</span>
                                        </label>
                                        <label class="flex items-center text-gray-700 text-sm hover:bg-gray-100 px-2 py-1.5 rounded cursor-pointer">
                                            <input type="radio" name="status" value="aktif" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                                                   {{ request('status') === 'aktif' ? 'checked' : '' }}>
                                            <span class="ml-2">Aktif</span>
                                        </label>
                                        <label class="flex items-center text-gray-700 text-sm hover:bg-gray-100 px-2 py-1.5 rounded cursor-pointer">
                                            <input type="radio" name="status" value="lulus" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                                                   {{ request('status') === 'lulus' ? 'checked' : '' }}>
                                            <span class="ml-2">Lulus</span>
                                        </label>
                                        <label class="flex items-center text-gray-700 text-sm hover:bg-gray-100 px-2 py-1.5 rounded cursor-pointer">
                                            <input type="radio" name="status" value="pindah" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300"
                                                   {{ request('status') === 'pindah' ? 'checked' : '' }}>
                                            <span class="ml-2">Pindah</span>
                                        </label>
                                    </div>
                                    <div class="mt-3 pt-3 border-t border-gray-200 flex justify-between items-center">
                                        <button type="submit" class="w-full px-3 py-1.5 bg-indigo-600 text-white text-xs font-semibold rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1">
                                            Terapkan Filter
                                        </button>
                                    </div>
                                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 border-collapse border border-gray-300">
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-600 uppercase tracking-wider border border-gray-300">Status</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($siswas as $index => $siswa)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm border border-gray-300 text-center">
                                            @if($siswa->status === \App\Models\Siswa::STATUS_AKTIF)
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                                    Aktif
                                                </span>
                                            @elseif($siswa->status === \App\Models\Siswa::STATUS_LULUS)
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                                                    Lulus
                                                </span>
                                            @elseif($siswa->status === \App\Models\Siswa::STATUS_PINDAH)
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium border border-gray-300 text-center">
                                            <div class="flex flex-wrap gap-2 justify-center">
                                                ...
                                                {{-- Replace the delete form with graduation form --}}
                                                <form action="{{ route('admin.siswa.destroy', $siswa) }}" method="POST" class="inline"
                                                      onsubmit="return confirm('Apakah Anda yakin ingin mengubah status siswa {{ $siswa->nama_siswa }} menjadi LULUS?');">
                                                    @csrf
                                                    @method('DELETE') {{-- atau 'PATCH' jika Anda membuat route baru untuk meluluskan --}}
                                                    <button type="submit"
                                                            class="inline-flex items-center px-3 py-1 border border-yellow-600 text-yellow-600 bg-white rounded-md text-xs font-semibold uppercase hover:bg-yellow-600 hover:text-white transition-colors duration-150">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center border border-gray-300">Data siswa tidak ditemukan.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
